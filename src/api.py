...
from src.forms import ClienteRegis, link_form_builder, proponente_form
from src.models import Cliente, Links, Propostas, Telefone
...
def gerar_link():
    ...
    if cli := Cliente.query.filter_by(
    ...
    current_app.db.session.commit()
    return render_block(
        "htmx/link_form.html", "c_link", form=form, link=url_for("lead.oi", hashdd=url)
    )
...
def permissoes(id):
    ...
    form = link_form_builder(
        [x.nome for x in Propostas.query.all()],
        **{prop.nome.upper(): True for prop in cliente.links[0].propostas},
    )
