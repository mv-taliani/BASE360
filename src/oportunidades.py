...
from src.forms import (MESES, DocumentoForm, Etapa1Form, Etapa2Form,
                       Etapa3Form, Etapa4Form, Etapa5Form, Etapa6Form,
                       Etapa7Form, Etapa8Form, EtapaFinalForm, RecebimentoForm,
                       proponente_form)
from src.models import (Arquivos, Detalhes, Instituição, Links, Preenchimento,
                        Propostas, Recebimento)
from src.utils import (adicionar_etapa, atualizar_preenchimento,
                       redirecionar_view, somente_cliente, upload_s3)

oportunidades = Blueprint("oportunidades", __name__)


@oportunidades.url_defaults
def oportunidades_url_defaults(endpoint, values):
    ...


@oportunidades.url_value_preprocessor
def get_oportunidade(endpoint, values: dict):
    oportunidade: str = values.pop("oportunidade")
    novo: bool = request.args.get("novo", False)
    if oportunidade in [prop.nome for prop in g.cliente.links[0].propostas]:
        g.oportunidade: str = oportunidade
        session["oportunidade_id"] = next(
            filter(lambda x: x.nome == oportunidade, g.cliente.links[0].propostas)
        ).id
        g.preenchimento: Preenchimento = (
            Preenchimento.query.join(Links)
            .filter_by(link=g.cliente.links[0].link)
            .join(Propostas)
            .filter_by(nome=oportunidade)
            .first()
        )
        if not g.preenchimento:
            g.preenchimento: Preenchimento = Preenchimento(
                link_id=g.cliente.links[0].id,
                proposta_id=next(
                    filter(
                        lambda x: x.nome == g.oportunidade, g.cliente.links[0].propostas
                    )
                ).id,
            )
...
def index():
    if g.oportunidade == "FIM":
        return redirect(
            url_for('.etapa3', hashdd=g.cliente.links[0].link,
                    oportunidade=g.oportunidade)
        )
    prop_form = proponente_form(g.oportunidade)
    alvo = url_for(
        "lead.oportunidades.index",
        hashdd=g.cliente.links[0].link,
        oportunidade=g.oportunidade,
    )
    if current_app.htmx:
        if prop_form.validate_on_submit():
            ...
            adicionar_etapa(
                ...
                prop_id=session["oportunidade_id"],
                etapa="0 - Proponente",
            )
        ...
        if request.method == "GET":
            template = render_template(
                "cadastro/proponente.html", prop_form=prop_form, alvo=alvo
            )
            return make_response(template, retarget="body", reswap="outerHTML")
        return render_block(
            "cadastro/proponente.html", "form", prop_form=prop_form, alvo=alvo
        )
...
def etapa4():
    if g.oportunidade == "FIM":
        return redirect(
            url_for(
                ".etapa5", hashdd=g.cliente.links[0].link, oportunidade=g.oportunidade
            )
        )
    prop_form = Etapa4Form()
    alvo = url_for(
        "lead.oportunidades.etapa4",
        hashdd=g.cliente.links[0].link,
        oportunidade=g.oportunidade,
    )
    if current_app.htmx:
...
def etapa5():
    ...
    alvo = url_for(
        "lead.oportunidades.etapa5_add",
        hashdd=g.cliente.links[0].link,
        oportunidade=g.oportunidade,
    )
    if [i for i in g.preenchimento.detalhes]:
        alvo2 = url_for(
            "lead.oportunidades.etapa6",
            hashdd=g.cliente.links[0].link,
            oportunidade=g.oportunidade,
        )
    else:
    ...
    return render_template(
        "cadastro/tabela.html", prop_form=prop_form, alvo=alvo, alvo2=alvo2
    )
    ...
    if current_app.htmx:
        if prop_form.validate():
            detalhe = Detalhes(
                **{
                    key: value
                    for key, value in prop_form.data.items()
                    if key != "csrf_token"
                }
            )
            ...
            return resposta
        return render_block(
            "cadastro/tabela.html", "table", prop_form=prop_form, alvo=alvo
        )
    return render_block(
        "cadastro/tabela.html", "content", prop_form=prop_form, alvo=alvo
    )
...
def add_data(id):
    ...
    if form.validate():
        recebimento = (
            Recebimento.query.filter_by(ano=form.ano.data, mes=form.mes.data)
            .join(Detalhes)
            .filter_by(id=id)
            .first()
        )
...
def etapa6():
    if g.oportunidade == "FIM":
        return make_response(
            redirect=url_for(
                ".etapa9", hashdd=g.cliente.links[0].link, oportunidade=g.oportunidade
            )
        )
...
def etapa8():
    ...
    if current_app.htmx:
        ...
        flash(
            "Nesta seção deverá ser preenchido a contrapartida social (Ou seja, dos 10% da verba destina ao CANS, 5% "
    ...
    if current_app.htmx:
        if prop_form.validate_on_submit():
            ...
            for arquivo in prop_form.docs.data:
                ...
                arquivo.filename = (
                        str(uuid.uuid4().hex) + f"{g.cliente.cpf}_{g.oportunidade}.pdf"
                )
...
def etapafinal():
    ...
    if current_app.htmx:
        if prop_form.validate():
            ...
            return make_response(
                redirect=url_for("lead.oi", hashdd=g.cliente.links[0].link)
            )
        if request.method == "GET":
            template = render_template(
                "cadastro/proponente.html", prop_form=prop_form, alvo=alvo
